...
// REUSE FUNCTIONS
async function getGrafanaFolderByName(folderName) {
...
async function downloadAndWriteDashboardFolder(grafanaFolderName, targetDir = "dashboards") {
    ...
    await Promise.all(dashboardFolderItems.map(async (dashboardFolderItem) => {
        const targetFilePath = `${targetDir}/${dashboardFolderItem.title
            .toLowerCase()
            .replaceAll(" ", "-")
            .replaceAll("(", "")
            .replaceAll(")", "")
...
async function readLocalDashboards(localDashboardFolder) {
...
async function uploadDashboardsToGrafana(localDashboardFolder) {
...
async function uploadDashboard(dashboard, grafanaFolderUid) {
    dashboard.id = null;
