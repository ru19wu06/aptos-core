...
import { AccountAddress, ChainId, RawTransaction, EntryFunction, StructTag, TypeTagVector, TransactionPayloadEntryFunction, Identifier, TypeTagStruct, TypeTagAddress, TypeTagBool, TypeTagU8, TypeTagU64, TypeTagU128, TypeTagSigner, Ed25519Signature, TransactionPayloadScript, Script, TransactionArgumentBool, TransactionArgumentU8, TransactionArgumentU64, TransactionArgumentAddress, TransactionArgumentU8Vector, TransactionArgumentU128, } from "../aptos_types";
import { HexString } from "../hex_string";
import { TransactionBuilderEd25519 } from "./builder";
// eslint-disable-next-line operator-linebreak
const VECTOR_FILES_ROOT_DIR = process.env.VECTOR_FILES_ROOT_DIR || path.resolve(__dirname, "..", "..", "..", "..", "..", "api", "goldens");
const ENTRY_FUNCTION_VECTOR = path.join(VECTOR_FILES_ROOT_DIR, "aptos_api__tests__transaction_vector_test__test_entry_function_payload.json");
const SCRIPT_VECTOR = path.join(VECTOR_FILES_ROOT_DIR, "aptos_api__tests__transaction_vector_test__test_script_payload.json");
function parseTypeTag(typeTag) {
    ...
    if (typeTag.struct) {
        ...
        const structTag = new StructTag(AccountAddress.fromHex(address), new Identifier(module), new Identifier(name), typeArgs);
...
function sign(rawTxn, privateKey) {
    ...
    const txnBuilder = new TransactionBuilderEd25519((signingMessage) => new Ed25519Signature(nacl.sign(signingMessage, signingKey.secretKey).slice(0, 64)), publicKey);
    return bytesToHex(txnBuilder.sign(rawTxn));
}
function verify(raw_txn, payload, private_key, expected_output) {
    const rawTxn = new RawTransaction(AccountAddress.fromHex(raw_txn.sender), BigInt(raw_txn.sequence_number), payload, BigInt(raw_txn.max_gas_amount), BigInt(raw_txn.gas_unit_price), BigInt(raw_txn.expiration_timestamp_secs), new ChainId(raw_txn.chain_id));
...
describe("Transaction builder vector test", () => {
    it("should pass on entry function payload", () => {
        const vector = JSON.parse(fs.readFileSync(ENTRY_FUNCTION_VECTOR, "utf8"));
        vector.forEach(({ raw_txn, signed_txn_bcs, private_key }) => {
            const payload = raw_txn.payload.EntryFunction;
            const entryFunctionPayload = new TransactionPayloadEntryFunction(EntryFunction.natural(`${payload.module.address}::${payload.module.name}`, payload.function, payload.ty_args.map((tag) => parseTypeTag(tag)), payload.args.map((arg) => new HexString(arg).toUint8Array())));
            verify(raw_txn, entryFunctionPayload, private_key, signed_txn_bcs);
        });
    });
    it("should pass on script payload", () => {
        const vector = JSON.parse(fs.readFileSync(SCRIPT_VECTOR, "utf8"));
        vector.forEach(({ raw_txn, signed_txn_bcs, private_key }) => {
            const payload = raw_txn.payload.Script;
            // payload.code is hex string
            const code = new HexString(payload.code).toUint8Array();
            const scriptPayload = new TransactionPayloadScript(new Script(code, payload.ty_args.map((tag) => parseTypeTag(tag)), payload.args.map((arg) => parseTransactionArgument(arg))));
