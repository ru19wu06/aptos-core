...
test("submits bcs transaction", async () => {
    ...
    const entryFunctionPayload = new TxnBuilderTypes.TransactionPayloadEntryFunction(TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(account2.address())), bcsSerializeUint64(717)]));
...
test("submits multisig transaction simulation", async () => {
    ...
    expect(accountResource.data.coin.value).toBe("0");
    const token = new TxnBuilderTypes.TypeTagStruct(TxnBuilderTypes.StructTag.fromString("0x1::aptos_coin::AptosCoin"));
    const entryFunctionPayload = new TxnBuilderTypes.TransactionPayloadEntryFunction(TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(account4.address())), bcsSerializeUint64(123)]));
...
test("submits bcs transaction simulation", async () => {
    ...
    const account1 = new AptosAccount();
    const account2 = new AptosAccount();
    const txns1 = await faucetClient.fundAccount(account1.address(), 100000000);
    const txns2 = await faucetClient.fundAccount(account2.address(), 100000000);
    const tx1 = await client.getTransactionByHash(txns1[0]);
    const tx2 = await client.getTransactionByHash(txns2[0]);
    expect(tx1.type).toBe("user_transaction");
    expect(tx2.type).toBe("user_transaction");
    const checkAptosCoin = async () => {
        ...
        const account1Resource = resources1.find((r) => r.type === aptosCoin);
        const account2Resource = resources2.find((r) => r.type === aptosCoin);
        expect(account1Resource.data.coin.value).toBe("100000000");
        expect(account2Resource.data.coin.value).toBe("100000000");
    };
    await checkAptosCoin();
    const token = new TxnBuilderTypes.TypeTagStruct(TxnBuilderTypes.StructTag.fromString("0x1::aptos_coin::AptosCoin"));
    const entryFunctionPayload = new TxnBuilderTypes.TransactionPayloadEntryFunction(TxnBuilderTypes.EntryFunction.natural("0x1::coin", "transfer", [token], [bcsToBytes(TxnBuilderTypes.AccountAddress.fromHex(account2.address())), bcsSerializeUint64(1000)]));
    const rawTxn = await client.generateRawTransaction(account1.address(), entryFunctionPayload);
    ...
    const account2AptosCoin = transactionRes.changes.filter((change) => {
        ...
        return (write.address === account2.address().toShortString() &&
            write.data.type === aptosCoin &&
            write.data.data.coin.value === "100001000");
    });
    expect(account2AptosCoin).toHaveLength(1);
    await checkAptosCoin();
}, longTestTimeout);
...
test("publishes a package", async () => {
    const client = new AptosClient(NODE_URL);
    const faucetClient = getFaucetClient();
    const account1 = new AptosAccount(new HexString("0x883fdd67576e5fdceb370ba665b8af8856d0cae63fd808b8d16077c6b008ea8c").toUint8Array());
    await faucetClient.fundAccount(account1.address(), 100000000);
    const txnHash = await client.publishPackage(account1, new HexString(
    ...
    "").toUint8Array(), [
        new TxnBuilderTypes.Module(new HexString(
        // eslint-disable-next-line max-len
...
test("rotates auth key ed25519", async () => {
    ...
    expect(HexString.fromUint8Array(bcsToBytes(origAddress)).hex()).toBe(HexString.fromUint8Array(bcsToBytes(aliceAddress)).hex());
}, longTestTimeout);
