...
test("full tutorial nft token flow", async () => {
    ...
    await client.waitForTransaction(await tokenClient.createTokenWithMutabilityConfig(alice, collectionName, tokenName, "Alice's simple token", 2, "https://aptos.dev/img/nyan.jpeg", 1000, alice.address(), 1, 0, ["TOKEN_BURNABLE_BY_OWNER"], [bcsSerializeBool(true)], ["bool"], [false, false, false, false, true]), { checkSuccess: true });
    const tokenId = {
        token_data_id: {
    ...
    let a = await tokenClient.mutateTokenProperties(alice, bob.address(), alice.address(), collectionName, tokenName, 0, 1, ["test"], [bcsSerializeBool(true)], ["bool"]);
